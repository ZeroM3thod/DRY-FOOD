...
export default function HomePage() {
  ...
  useEffect(() => { document.body.style.overflow = activeModal ? 'hidden' : '' }, [activeModal])
  ...
  const openModal = (type: ModalType) => { setActiveModal(type); setMenuOpen(false) }
  const closeModal = () => { setActiveModal(null); setShowForgot(false); setLoginError(''); setForgotMsg(''); setSignupError(''); setSignupSuccess(false) }
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    const { error } = await supabase.auth.signUp({ email: signupEmail, password: signupPassword, options: { data: { full_name: signupName, referral_code: signupReferral }, emailRedirectTo: `${window.location.origin}/dashboard` } })
    if (error) { setSignupError(error.message); setSignupLoading(false); return }
    setSignupSuccess(true); setSignupLoading(false)
  }

  async function handleForgotPassword(e: React.FormEvent) {
    e.preventDefault(); setForgotLoading(true); setForgotMsg('')
    const { error } = await supabase.auth.resetPasswordForEmail(forgotEmail, { redirectTo: `${window.location.origin}/auth/reset-password` })
    setForgotLoading(false); setForgotMsg(error ? error.message : 'Password reset link sent! Check your inbox.')
  }
  ...
  return (
    <>
      ...
      <nav id="navbar">
        <div className="nav-inner">
          ...
          <div className="nav-actions">
            ...
            <button className="btn-ghost" onClick={() => openModal('signup')}>Sign Up</button>
      ...
      <section className="hero">
        <div className="hero-inner">
          ...
          <div className="hero-cta">
            <button className="btn-lg" onClick={() => openModal('signup')}>Start Investing</button>
      ...
      <section className="seasons" id="seasons">
        <div className="seasons-inner">
          <div className="seasons-header reveal">
            ...
            <button className="btn-primary" onClick={() => openModal('signup')}>Join Season 4</button>
          </div>
          <div className="seasons-grid reveal">
            ...
            <div className="season-card active" style={{ gridColumn: 'span 3' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '20px', alignItems: 'start' }}>
                ...
                <button className="btn-primary" style={{ whiteSpace: 'nowrap', padding: '12px 28px' }} onClick={() => openModal('signup')}>Invest Now</button>
              </div>
      ...
      <section className="referral">
        <div className="referral-inner">
          <div className="reveal">
            ...
            <br />
            <button className="btn-lg" onClick={() => openModal('signup')} style={{ marginTop: '8px' }}>Get My Referral Code</button>
          </div>
        ...
        <div className="about-inner">
          ...
          <div className="about-aside reveal">
            <div className="aside-card">
              ...
              <button className="aside-cta" onClick={() => openModal('signup')}>Open an Account</button>
      ...
      <footer>
        <div className="footer-inner">
          <div className="footer-top">
            ...
            <div className="footer-col"><h5>Platform</h5><ul><li><a href="#seasons">Seasons</a></li><li><a onClick={() => openModal('signup')}>Start Investing</a></li><li><a href="#referral">Referral Programme</a></li><li><a href="/dashboard">Portfolio Tracker</a></li></ul></div>
      ...
      <div className={`modal-overlay ${activeModal === 'login' ? 'active' : ''}`} onClick={e => { if (e.target === e.currentTarget) closeModal() }}>
        <div className="modal">
          <div className="modal-head"><h3>{showForgot ? 'Reset Password' : 'Welcome Back'}</h3><button className="modal-close" onClick={closeModal}>✕</button></div>
          <div className="modal-body">
            {!showForgot ? (
              <form onSubmit={handleLogin} style={{ display: 'contents' }}>
                <div className="modal-input"><label>Email Address</label><input type="email" placeholder="you@example.com" required value={loginEmail} onChange={e => setLoginEmail(e.target.value)} /></div>
                <div className="modal-input"><label>Password</label><input type="password" placeholder="••••••••" required value={loginPassword} onChange={e => setLoginPassword(e.target.value)} /></div>
                <button type="button" className="modal-link" onClick={() => setShowForgot(true)}>Forgot password?</button>
                {loginError && <div className="modal-error">{loginError}</div>}
                <button type="submit" className="modal-submit" disabled={loginLoading}>{loginLoading ? 'Signing In...' : 'Sign In'}</button>
                <p className="modal-switch">Don&apos;t have an account? <a onClick={() => { setActiveModal('signup'); setLoginError('') }}>Create one</a></p>
              </form>
            ) : (
              <form onSubmit={handleForgotPassword} style={{ display: 'contents' }}>
                <div className="modal-input"><label>Email Address</label><input type="email" placeholder="you@example.com" required value={forgotEmail} onChange={e => setForgotEmail(e.target.value)} /></div>
                {forgotMsg && <div className={forgotMsg.includes('sent') ? 'modal-success' : 'modal-error'}>{forgotMsg}</div>}
                <button type="submit" className="modal-submit" disabled={forgotLoading}>{forgotLoading ? 'Sending...' : 'Send Reset Link'}</button>
                <p className="modal-switch"><a onClick={() => setShowForgot(false)}>← Back to Sign In</a></p>
              </form>
            )}
          </div>
        </div>
      ...
      {/* SIGNUP MODAL */}
      <div className={`modal-overlay ${activeModal === 'signup' ? 'active' : ''}`} onClick={e => { if (e.target === e.currentTarget) closeModal() }}>
        <div className="modal">
          <div className="modal-head"><h3>Create Account</h3><button className="modal-close" onClick={closeModal}>✕</button></div>
          <div className="modal-body">
            {signupSuccess ? (
              <div className="modal-success">✅ Account created! Please check your email to verify your account, then sign in.</div>
            ) : (
              <form onSubmit={handleSignup} style={{ display: 'contents' }}>
                <div className="modal-input"><label>Full Name</label><input type="text" placeholder="Rafiqul Molla" required value={signupName} onChange={e => setSignupName(e.target.value)} /></div>
                <div className="modal-input"><label>Email Address</label><input type="email" placeholder="you@example.com" required value={signupEmail} onChange={e => setSignupEmail(e.target.value)} /></div>
                <div className="modal-input"><label>Password</label><input type="password" placeholder="Create a strong password" required value={signupPassword} onChange={e => setSignupPassword(e.target.value)} /></div>
                <div className="modal-input"><label>Referral Code (Optional)</label><input type="text" placeholder="Enter code to earn 5% bonus" value={signupReferral} onChange={e => setSignupReferral(e.target.value)} /></div>
                {signupError && <div className="modal-error">{signupError}</div>}
                <button type="submit" className="modal-submit" disabled={signupLoading}>{signupLoading ? 'Creating Account...' : 'Get Started'}</button>
                <p className="modal-switch">Already have an account? <a onClick={() => { setActiveModal('login'); setSignupError('') }}>Sign in</a></p>
              </form>
            )}
          </div>
        </div>
